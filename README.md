...
```powershell
powershell -ExecutionPolicy Bypass -Command "Invoke-WebRequest -Uri 'http://<panel-url>/install-agent.ps1' -OutFile 'bsdock-install.ps1' -UseBasicParsing; .\bsdock-install.ps1 -PanelURL '<panel-url>' -Token '<token>'"
```

> 该命令会把安装脚本下载到当前目录并执行。请在 PowerShell 或 Git Bash 中粘贴执行。
