...
> 状态：待实现计划
>
> **设计变更说明**：原始需求中前端框架为 React + Next.js，经确认后切换为 **Vite + React**，以配合 `bun + vite` 的构建方式。其余技术约束（shadcn/ui、Tailwind CSS、默认深色主题）保持不变。
...
- **Panel-Backend**：Go（Gin）提供 HTTP API、WebSocket 服务，使用 SQLite 持久化。
- **Agent**：Go 编译为 linux/amd64 与 linux/arm64 静态二进制，运行在 Node 上，主动向 Panel-Backend 建立 WebSocket 连接。
...
- 令牌仅用于 Agent 首次 WebSocket 注册；后端验证签名与有效期、解析出 `node_id` 后，立即将该 token 标记为已使用，不可再次注册。
- 安装命令示例：
...
- Agent 与后端通信默认使用 WSS（WebSocket Secure），WSS 不可用时允许回退 WS 仅在明确 `--insecure` 模式下。
...
4. **注册**：Agent 携带 token 通过 WebSocket 连接 `/api/v1/agent/ws`，发送 `register` 消息并附带系统信息。
5. **在线**：后端验证 token 通过后，将 Node 状态改为 `online`，保存系统信息，向前端广播更新。
6. **心跳**：Agent 每 30 秒发送 `heartbeat` 消息；后端超过 60 秒未收到则标记 `offline`。
7. **重连**：Agent 断线后自动重连，重连间隔 5s/10s/30s 指数退避。

## 7. Agent-Backend 协议

Agent 与后端通过 WebSocket 交换 JSON 消息。
...
- 前端优先通过 `/api/v1/ws` 建立 WebSocket 接收 `node_update`。
...
- **Linux VPS**：上传 `panel` 二进制与 `scripts/install-panel.sh` 到服务器，执行 `install-panel.sh` 完成 systemd 注册、初始化数据库、启动服务。
